...
class Database:
    ...
    def _get_raw_data(self):
        try:
            ...
            # هيكل البيانات الكامل ليطابق جداولك الأصلية
            return {
                "users": {}, "conversations": [], "games": [], 
                "reports": [], "gifts": [], "stats": {"total_chats": 0}
            }, None
        except: 
            return {"users": {}, "conversations": [], "stats": {}}, None

    def _save(self, data, sha, msg="Update"):
        try:
    ...
    # --- إدارة المستخدمين (مطابق لقاعدتك الأصلية) ---
    def create_user(self, info):
        ...
        if uid not in data["users"]:
            ...
            data["users"][uid] = {
                ...
                "vip_until": 0, "vip_level": 0, "vip_title": "",
                "total_chats": 0, "join_ts": int(time.time()), "referral_code": ref_code,
                "last_reward_ts": 0, "partner": None
            }
    ...
    # --- الإحصائيات (التي تظهر في لوحة التحكم) ---
    def get_stats(self):
        data, _ = self._get_raw_data()
        u = data.get("users", {})
        now = int(time.time())
