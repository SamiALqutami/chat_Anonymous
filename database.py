...
class Database:
    ...
    def _save(self, data, sha, msg="Database Sync"):
        try:
    ...
    def create_user(self, info):
        ...
        if uid not in data["users"]:
            ...
            data["users"][uid] = {
                ...
                "age": "غير محدد", "country": "غير محدد", "vip_until": 0,
    ...
    # --- حل مشكلة البحث وإظهار البيانات ---
    ...
    def add_vip_days(self, user_id, days):
        ...
        uid = str(user_id)
        if uid in data["users"]:
            current_vip = max(data["users"][uid].get("vip_until", 0), int(time.time()))
            data["users"][uid]["vip_until"] = current_vip + (days * 86400)
            self._save(data, sha)
            return True
        return False
